  ...
  <div>
    <el-card style="margin: 20px 0px">
      <Conmmon
        ...
        :show="!state.isShowTable"
      ></Conmmon>
    </el-card>
    <el-card>
      <div v-show="state.isShowTable">
        <el-button
          type="primary"
          icon="el-icon-plus"
        ...
        >
        <!-- 表格:展示平台属性 -->
        <el-table style="width: 100%" border :data="state.attrList">
      ...
      <div v-show="!state.isShowTable">
        <el-form
          ...
          ref="form"
          label-width="80px"
          :model="state.attrInfo"
        >
          <el-form-item label="属性名">
            <el-input
              placeholder="请输入属性名"
              v-model="state.attrInfo.attrName"
            ></el-input>
          </el-form-item>
        </el-form>
        <el-button
          ...
          icon="el-icon-plus"
          @click="addAttrValue"
          :disabled="!state.attrInfo.attrName"
          >添加属性值</el-button
        >
        ...
        <el-table
          ...
          border
          :data="state.attrInfo.attrValueList"
        >
        ...
        <el-button
          ...
          :disabled="state.attrInfo.attrValueList.length < 1"
          >保存</el-button
        >
        <el-button @click="isShowTable = true">取消</el-button>
      </div>
...
async function getAttrList() {
  //获取分类的ID
  //获取属性列表的数据
  let { data } = await API.product.reqAttrList(
    state.category1Id,
    state.category2Id,
    state.category3Id
  );
  if (data.code == 200) {
    state.attrList = data.data;
  }
}
//添加属性值回调
function addAttrValue() {
  ...
  //valueName:相应的属性值的名称
  state.attrInfo.attrValueList.push({
    attrId: state.attrInfo.id, //对于修改某一个属性的时候，可以在已有的属性值基础之上新增新的属性值（新增属性值的时候，需要把已有的属性的id带上）
    valueName: "",
    flag: true,
  });
...
function addAttr() {
  ...
  state.attrInfo = {
    ...
    categoryId: state.category3Id, //三级分类的id
    categoryLevel: 3, //因为服务器也需要区分几级id
  };
}
function updateAttr(row) {
  ...
  state.attrInfo = cloneDeep(row);
  //在修改某一个属性的时候，将相应的属性值元素添加上flag这个标记
  state.attrInfo.attrValueList.forEach((item) => {
...
function toLook(row) {
  ...
  //   }
  //新增的属性值不能与已有的属性值重复
  let isRepat = state.attrInfo.attrValueList.some((item) => {
    //需要将row从数组里面判断的时候去除
...
function deleteAttrValue(index) {
  //当前删除属性值的操作是不需要发请求的
  state.attrInfo.attrValueList.splice(index, 1);
}
//保存按钮：进行添加属性或者修改属性的操作
async function addOrUpdateAttr() {
  //整理参数:1,如果用户添加很多属性值，且属性值为空的不应该提交给服务器
  //提交给服务器数据当中不应该出现flag字段
  state.attrInfo.attrValueList = state.attrInfo.attrValueList.filter((item) => {
    //过滤掉属性值不是空的
    if (item.valueName != "") {
      //删除掉flag属性
      delete item.flag;
  ...
  try {
    //发请求
    await API.product.reqAddOrUpdateAttr(state.attrInfo);
    //展示平台属性的信号量进行切换
